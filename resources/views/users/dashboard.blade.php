    ...
    <div class="mb-6">
        ...
        @foreach($genres as $genre)
            <a href="{{ route('books.byGenre', $genre->name) }}"
               class="inline-block px-3 py-1 rounded bg-gray-200 text-gray-700 hover:bg-blue-500 hover:text-white transition mr-2 mb-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 active:bg-blue-600 active:text-white
               {{ isset($selectedGenre) && $selectedGenre->id === $genre->id ? 'bg-blue-600 text-white' : '' }}">
                {{ $genre->name }}
            </a>
        @endforeach
        <a href="{{ route('dashboard') }}"
           class="inline-block px-3 py-1 rounded bg-gray-300 text-gray-700 hover:bg-blue-500 hover:text-white transition mb-2">
            All
        </a>
    </div>
